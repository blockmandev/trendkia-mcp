...
check(m._norm_lang(None) == "hi", "None -> hi, the site's own language")
check(m._norm_lang("klingon") == "hi", "unknown -> hi, not an exception")
check(m._norm_lang(None, "en") == "en", "an explicit fallback wins -- this is how a URL's own language is honoured")
check(m._norm_lang("hi", "en") == "hi", "an explicit lang beats the fallback, so either edition stays selectable")
check(m.DEFAULT_LANG == "hi", "last-resort default is hi; the URL normally decides before this is reached")
